...
#[component]
fn ClearBroadcastButton() -> impl IntoView {
    let ctx = use_context::<AppContext>().expect("AppContext");
            ...
            {move || {
                results.get().into_iter().map(|hit| {
                    ...
                    let on_click = {
                        let bs = bs.clone();
                        move |_| {
                            // Set the book and reference inputs from the search result
...
#[component]
fn LoadedPassagesHistory() -> impl IntoView {
    let bs = use_context::<BibleState>().expect("BibleState");
                ...
                Some(view! {
                    <div class="bible__history" data-role="bible-history">
                        ...
                        {items.into_iter().map(|entry| {
                            ...
                            let on_click = move |_| {
                                // Restore the reference from history
                                let books = bs.books.get_untracked();
