...
- A dedicated OBS-ready timer overlay is a stream-graphics output at `/stream/timer` (designable in the stream editor at `/ui/stream`); the legacy `/overlays/timer` URL 302-redirects to it, so existing OBS browser sources keep working.
