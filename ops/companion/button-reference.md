...
| 6 | `Set HH:MM` | `timer.set_countdown_target` | Macro forwards the button text (HH:MM time-of-day) | Displays `timer_countdown_target` |
...
| Button | Label | Command | Payload | Feedback |
|--------|-------|---------|---------|----------|
| 1 | `Lyrics SNV` | `stage.layout` | `{ "code": "worship-snv" }` | Highlights green when `stage_layout_code === "worship-snv"` |
| 2 | `Lyrics PP` | `stage.layout` | `{ "code": "worship-pp" }` | Highlights when active |
| 3 | `Timer` | `stage.layout` | `{ "code": "timer" }` | Highlights when active |
| 4 | `Preach` | `stage.layout` | `{ "code": "preach" }` | Highlights when active |
...
`song_name` and `band_name` variables mirror the active song title (prefix removed) and its library for use in feedback or text buttons.
