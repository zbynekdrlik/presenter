...
| AI status on `/healthz` + `/ai/status` (#760): ONE shared `evaluate_ai_status`; backend-agnostic `{connected,error,model}` (no OAuth fields, OpenRouter-ready); live 3s-bounded probe (no on-disk cache); render code in `router/ai_health.rs` not the over-cap `router/ai.rs`; deploy gate `connected:false`→non-blocking `::error::`, `modelValid:false`→hard-fail | `.claude/rules/ai-health-endpoint.md` (auto-loads on `router/ai.rs`, `router/ai_health.rs`, `router.rs`) |
