...
| AI backend config (#761): OpenRouter via env; `PRESENTER_AI_*` WIN over the DB `ai-settings` row on the EFFECTIVE path only (`resolve_effective_settings` + `router/ai_env.rs`), never the raw persist/display path; `requiresClaudeAuth` recomputed on the post-override URL vs the fixed `BUNDLED_PROXY_PLACEHOLDER`; deploy writes 0600 `/etc/presenter/ai.env` (`EnvironmentFile=-`) from `OPENROUTER_API_KEY`+`AI_MODEL` in all three workflows; `DEFAULT_AI_MODEL`=OpenRouter slug; attribution headers in `ai/client.rs` | `.claude/rules/ai-backend-config.md` (auto-loads on `router/ai.rs`, `router/ai_env.rs`, `ai/mod.rs`, `ai/client.rs`, the 3 deploy workflows, the 2 units) |
