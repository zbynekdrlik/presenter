...
test('operator manages Bible workflow end-to-end', async ({ page, request }) => {
  await expect(async () => {
  ...
  await expect(async () => {
    const firstBook = await page.evaluate(() => window.__presenterBibleState.books[0]);
    ...
    expect(Number(firstBook.number ?? firstBook.book_number)).toBeGreaterThan(0);
  }).toPass();

  const translationButtons = page.locator('[data-role="translation-list"] button');
  ...
  const firstSlide = await page.evaluate(() => window.__presenterBibleState.slides[0]);
  ...
  expect(
    firstSlide.metadata && firstSlide.metadata.bible && (firstSlide.metadata.bible.bookCode || firstSlide.metadata.bible.book_code)
  ).toBe('JHN');
  expect(
    firstSlide.metadata && firstSlide.metadata.bible && (firstSlide.metadata.bible.bookNumber ?? firstSlide.metadata.bible.book_number)
  ).toBe(43);

  await expect(async () => {
    const historyCount = await page.evaluate(() => window.__presenterBibleState.loadedPassages.length);
    expect(historyCount).toBeGreaterThan(0);
  }).toPass();

  const loadedPassageItems = page.locator('[data-role="loaded-passages"] .operator__list-item');
  await expect(loadedPassageItems.first()).toBeVisible();

  const storedPassage = await page.evaluate(() => window.__presenterBibleState.loadedPassages[0]);
  expect(storedPassage).toBeTruthy();
  expect(storedPassage.bookCode || storedPassage.book_code).toBe('JHN');
  expect(Number(storedPassage.bookNumber ?? storedPassage.book_number)).toBe(43);
  ...
  await page.locator('[data-role="slide-translation"]').first().fill(customTranslation);
  await page.locator('[data-role="slide-main-ref"]').first().fill(customReference);

  await expect(async () => {
    const slide = await page.evaluate(() => window.__presenterBibleState.slides[0]);
  ...
  const triggerResponse = await request.post(new URL('/bible/trigger', baseURL).toString(), {
    data: triggerPayload,
  });
  expect(triggerResponse.ok()).toBeTruthy();

  const referenceLabel = broadcast.start === broadcast.end
    ? `${broadcast.book} ${broadcast.chapter}:${broadcast.start}`
    : `${broadcast.book} ${broadcast.chapter}:${broadcast.start}-${broadcast.end}`;

  await expect(page.locator('.operator__active-card strong')).toHaveText(referenceLabel, {
    timeout: 15_000,
  });
  ...
  const activeVerseStart = active.passage.reference.verse_start ?? active.passage.reference.verseStart;
  const activeVerseEnd = active.passage.reference.verse_end ?? active.passage.reference.verseEnd;
  ...
  await expect(page.locator('.operator__active-card strong')).toHaveText('No active passage', {
    timeout: 10_000,
  });

  await page.locator('[data-role="slide-select"]').first().uncheck({ force: true });
  await page.locator('[data-role="select-all-slides"]').click();
  await expect(page.locator('[data-role="selection-count"]')).toHaveText(`${slideCount} selected`);
  ...
  await expect(async () => {
    const listText = await page.locator('[data-role="presentations-list"]').innerText();
    expect(listText).toContain(presentationName);
  }).toPass({ timeout: 10_000 });

  const presentationsResponse = await request.get(new URL('/bible/presentations', baseURL).toString());
  expect(presentationsResponse.ok()).toBeTruthy();
  const presentations = await presentationsResponse.json();
  const createdSummary = presentations.find((entry: any) => entry.name === presentationName);
  ...
  const detailMainReference = detail.slides[0].main_reference ?? detail.slides[0].mainReference;
  expect(detailMainReference).toBe(customReference);

  await page.locator('[data-role="slide-select"]').first().uncheck({ force: true });
  const checkboxCount = await page.locator('[data-role="slide-select"]').count();
  const targetCheckbox = checkboxCount > 1
    ? page.locator('[data-role="slide-select"]').nth(1)
    : page.locator('[data-role="slide-select"]').first();
  await targetCheckbox.check({ force: true });
  await page.selectOption('[data-role="presentation-select"]', createdSummary.id);
  await page.locator('[data-role="presentation-add"]').click();
  await expect(page.locator('[data-role="toast"]')).toHaveAttribute('data-visible', 'true');

  const appendedDetailResponse = await request.get(new URL(`/bible/presentations/${createdSummary.id}`, baseURL).toString());
  ...
  await expect(async () => {
    const state = await page.evaluate(() => window.__presenterBibleState.presentations);
  ...
  await expect(async () => {
    const preferences = await page.evaluate(() => window.__presenterBibleState.preferences);
  ...
  await expect(async () => {
    const state = await page.evaluate(() => {
      ...
      const translation = first?.metadata?.bible?.translation_code
