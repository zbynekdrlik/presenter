...
async function pickSlideWithContent(
  request: APIRequestContext,
  baseURL: string,
) {
  const librariesResp = await request.get(
    ...
    for (const presentation of library.presentations) {
      ...
      for (const slide of detail.presentation.slides) {
        ...
        if (stage || main || translation) {
          ...
          const fallbackSlide = detail.presentation.slides.find(
            (candidate) =>
...
test.describe("Operator control surface", () => {
  test("can trigger slide and drive timers with stage updates", async ({
    page,
    context,
  }) => {
    await expect(async () => {
    ...
    await expect(updatedRow.locator(".operator__list-label")).toHaveText(
      renamedLibraryName,
    );
    await editButton.click();
    await expect(libraryModal).toHaveAttribute("data-open", "true");
    const favoriteToggleAfterSave = page.locator(
    ...
    await editButton.click();
    await expect(libraryModal).toHaveAttribute("data-open", "true");
    const favoriteToggleAfterUnpin = page.locator(
      '[data-role="library-edit-favorite"]',
    );
    ...
    const presentationCreateButton = page.locator(
    ...
    await expect(
      page.locator('[data-role="playlist-edit-parent"]'),
    ).toHaveCount(0);
    ...
    await expect(page.locator('[data-role="context-title"]').nth(0)).toHaveText(
      `Playlist: ${playlistName}`,
    );
    ...
    await presentationCreateButton.click();
    const separatorItem = page.locator(
    ...
    const libraryPresentationItems = page.locator(
      '[data-role="presentation-list"] [data-role="presentation-item"][data-type="presentation"]',
    );
    const playlistCountBadge = playlistButtonById
      .locator('[data-role="playlist-count"]')
      .first();
    const initialPlaylistCount = Number(
      (await playlistCountBadge.textContent())?.trim() || "0",
    );
    await expect(async () => {
    ...
    const presentationIds = (
      await libraryPresentationItems.evaluateAll((nodes) =>
        nodes
          .slice(0, 2)
          .map((node) => node.getAttribute("data-presentation-id") || ""),
      )
    ).filter((value) => value);
    ...
    const playlistItems = page.locator(
      '[data-role="catalog-bottom"] [data-role="presentation-item"][data-type="presentation"]',
    );
    ...
    await expect(playlistItems.nth(0).locator("span").first()).toContainText(
      secondPlaylistLabel.trim(),
      { timeout: 5_000 },
    );
    ...
    await expect(async () => {
      const count = await slideContainer
        .locator(".operator__slide-text--main")
        .count();
      if (count === 0) {
    ...
    await expect(async () => {
      const count = await slideContainer
        .locator(".operator__slide-text--translation")
        .count();
      if (count === 0) {
    ...
    await expect(async () => {
      const count = await slideContainer
        .locator(".operator__slide-text--stage")
        .count();
      if (count === 0) {
    ...
    if (mainTokens.length >= 1) {
      ...
      await expect(async () => {
        const count = await searchResults
    ...
    const searchPresentationResults = searchResults.locator(
    ...
    for (let index = 0; index < candidateCount; index += 1) {
      ...
      const candidateId =
        (await candidate.getAttribute("data-presentation-id")) || "";
      const candidateTitle = (
        await candidate.locator(".operator__search-result-title").innerText()
      ).trim();
    ...
    const firstSearchResult = searchResults
      .locator('[data-role="search-result-item"]')
      .first();
    await expect(firstSearchResult).toBeVisible({ timeout: 10_000 });
    ...
    const livePlaylistItems = page.locator(
      '[data-role="catalog-bottom"] [data-role="presentation-item"][data-type="presentation"]',
    );
    await expect(async () => {
    ...
    await expect
      .poll(async () =>
        (
          await livePlaylistItems.first().locator("span").first().innerText()
        ).trim(),
      )
      .toBe(liveRenameLabel);

    const refreshedRenameButton = livePlaylistItems
      .first()
    ...
    await expect(
      slideContainer.locator('[data-action="duplicate"]'),
    ).toHaveCount(0);
    ...
    const newTranslationTextarea = newSlideCard.locator(
      'textarea[data-field="translation"]',
    );
    const warningBanner = newSlideCard.locator('[data-role="slide-warning"]');

    await newMainTextarea.fill(
      "This line definitely exceeds twelve characters",
    );
    await newMainTextarea.blur();
    await expect(
      newSlideCard.locator(".operator__slide-text--main"),
    ...
    await expect(warningBanner).toContainText(
      "Main text exceeds 12 characters",
    );
    ...
    await page.waitForLoadState("networkidle");
    await expect(
      newSlideCard.locator(".operator__slide-text--translation"),
    ).toHaveAttribute("data-warning", "true");
    ...
    await expect(
      newSlideCard.locator(".operator__slide-text--translation"),
    ).toHaveAttribute("data-warning", "false", {
      timeout: 10_000,
    });
    ...
    await expect(slideButton.locator('[data-action="duplicate"]')).toHaveCount(
      1,
    );

    await page.locator('[data-role="mode-toggle"][data-mode="live"]').click();
    if (newSlideId) {
      const liveOrderBefore = await page.evaluate(
        ({ presentationId }) => {
          const helpers = (window as any).__presenterOperatorTestHelpers;
      ...
      if (liveOrderBefore.length > 1) {
        const desiredOrder = [
          newSlideId,
          ...liveOrderBefore.filter((id) => id !== newSlideId),
        ];
        await page.evaluate(
          ({ presentationId, ordered }) => {
            const helpers = (window as any).__presenterOperatorTestHelpers;
    ...
    await timerOverlayPage.waitForSelector("#timer-value", {
      state: "attached",
    });
    ...
    await expect(currentTextLocator).not.toHaveText(/\bNo active slide\b/, {
      timeout: 2_000,
    });
    ...
    await expect(page.locator('[data-role="stage-current"]')).toContainText(
      initialRegex,
      {
        timeout: 2_000,
        ignoreCase: true,
    ...
    await expect(currentTextLocator).toContainText(initialRegex, {
      timeout: 5_000,
    });
    ...
    await expect(page.locator('[data-role="stage-status"]')).toHaveAttribute(
      "data-active",
      "false",
      {
        timeout: 2_000,
    ...
    await expect(async () => {
      const layoutCode = await stagePage.evaluate(
        () => document.body.dataset.layoutCode,
      );
      if (layoutCode !== "timer") {
    ...
    await expect(async () => {
      ...
      if (
        ...
        state.display.toLowerCase() !== "running"
      ) {
    ...
    await expect(async () => {
      const overlayValue = await timerOverlayPage
        .locator("#timer-value")
        .innerText();
    ...
    const timersSnapshot = await page.evaluate(
      () => (window as any).__presenterTimers,
    );
    const initialRemaining =
      timersSnapshot?.countdownToStart?.secondsRemaining ?? null;
    ...
    await expect(async () => {
      ...
      if (typeof initialRemaining === "number") {
        if (updatedRemaining <= initialRemaining + 250) {
          throw new Error(
            `remaining did not increase enough: ${updatedRemaining}`,
          );
        }
      }
    ...
    await page
      .locator('[data-role="view-toggle"][data-view="worship"]')
      .click();
    ...
    await test.step("delete playlist and verify removal", async () => {
      ...
      const playlistEditButton = playlistRow.locator(
        '[data-action="playlist-edit"]',
      );
      await playlistEditButton.click();
      const playlistEditModal = page.locator(
