...
async function createScene(
  ...
  // Create body is {name, kind} only — `position` is server-assigned, never
  // client-settable (verified stream wire contract, .claude/rules/stream-graphics.md).
...
async function addElement(
  ...
  // wrapper, and z_order is always server-assigned (never client-settable).
