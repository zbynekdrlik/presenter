...
async function openStageDisplay(
  ...
  if (options?.forceLegacy) {
    await stagePage.addInitScript(() => {
      window.PRESENTER_STAGE_TEST_CONFIG = Object.assign(
        {},
        window.PRESENTER_STAGE_TEST_CONFIG,
        {
...
async function expectMonitorToReport(
  page: Page,
  expected: { connected: number; issues: number },
) {
  ...
  const counts = await page.evaluate(() =>
    window.__presenterOperatorTestHelpers?.stageMonitorCounts?.(),
  );
...
test("stage monitor handles legacy client identifiers", async ({
  page,
  context,
}) => {
