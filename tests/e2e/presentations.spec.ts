...
test('slide CRUD, reorder, and content update via presentations API', async ({ request }) => {
  ...
  const presPayload: { presentation: { id: string; name: string; slides: Array<{ id: string }> } } = await presResp.json();
  ...
  const dupResp = await request.post(
    new URL(`/presentations/${presentationId}/slides/${firstSlideId}/duplicate`, baseURL).toString(),
  );
  ...
  const updateResp = await request.patch(
    new URL(`/presentations/${presentationId}/slides/${secondSlideId}`, baseURL).toString(),
    {
      data: {
        main: 'Main line',
        translation: 'Preklad',
  ...
  const beforePayload: { presentation: { slides: Array<{ id: string }> } } = await detailBefore.json();
  const reversed = [...beforePayload.presentation.slides].reverse().map((s) => s.id);
  const reorderResp = await request.post(
    new URL(`/presentations/${presentationId}/slides/reorder`, baseURL).toString(),
  ...
  const delResp = await request.delete(
    new URL(`/presentations/${presentationId}/slides/${deleteTarget}`, baseURL).toString(),
  );
  ...
  const detailAfter = await request.get(new URL(`/presentations/${presentationId}`, baseURL).toString());
