...
## Uploaded web fonts (#778) — table + FontStore + ttf-parser + font-wait
...
- **Magic-byte gate:** accept only raw ttf (`00 01 00 00` / `true`) + otf (`OTTO`); reject `.ttc` (`ttcf`), woff/woff2 (`wOFF`/`wOF2`) → 422 (a browser can't `@font-face` a TTC and ttf-parser can't read woff2). `"fonts"` is a RESERVED slug (`/stream/fonts` + `/stream/fonts.css` shadow `/stream/{slug}`).
- **`ttf-parser` 0.25 API** (presenter-server ONLY, never core/WASM — bloats the WASM bundle): `Face::parse(bytes, 0u32) -> Result<_, FaceParsingError>`; family = `name_id::TYPOGRAPHIC_FAMILY` → fallback `FAMILY`, iterate `face.names()` by index (`names.len()` is u16, `names.get(i)`), `Name{name_id:u16,is_unicode(),to_string()->Option<String>}` (`to_string` needs the default `std` feature); `weight().to_number()->u16`; `is_italic()->bool`.
