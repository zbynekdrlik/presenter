...
## Adding a new element KIND (#753 added `color`) — every exhaustive-match site (Tier-0: no local compiler catches E0004)

A new `StreamElementProps` variant breaks these EXHAUSTIVE matches; grep `git grep -n 'StreamElementProps::' crates/` and fix ALL before pushing (CI's clippy job ~5 min in is the only thing that catches a miss):
- core `stream.rs`: `kind_str()` + `validate_props()` arms. `validate_color` (private) accepts `#rrggbb` OR `#rrggbbaa` — the ONE canonical color validator; reuse it (a superset of a strict-6-digit spec), don't add a parallel one. `stream_elements.kind` is a FREE `text()` column (no CHECK) → a new kind needs **NO migration**.
- output `components/stream/scene_render.rs`: the `match el.props` dispatch → add a new per-kind component (`components/stream/element_<kind>.rs` + a `pub mod` in `stream/mod.rs`). A static fill (no live content) mirrors `ElementImage`: an absolute `div.stream-element.stream-element--<kind>` with inline `left/top/width/height/…/z-index` from the `Frame` + `z_order`; `.stream-element` already gives `position:absolute; overflow:hidden`.
- editor `components/stream_editor/props_access.rs`: the `|`-patterns in `read_frame`/`with_frame_mut` (EVERY kind has a `Frame`); and `read_transition`/`with_transition_mut` — a kind WITHOUT a `content_transition` joins the `Image { .. } | <New> { .. }` no-transition arm.
- editor `props_access.rs::default_element_props`: add the `"<kind>" =>` arm **BEFORE the defensive `_ => Verse` fallback** — a missing arm compiles fine and SILENTLY makes the add-button create a Verse element (the trap this rule exists for; there is no E0004 here because the match is on a `&str`, not the enum).
- editor: `editor_panel.rs` `AddElementButton` (`kind="<kind>"`) + a `kind_label_sk` arm; a `<Show when=move || kind()=="<kind>">` fields section in `element_form.rs`; if the kind is STATIC, widen the `TransitionFields` guard (`kind() != "image" && kind() != "color" && …`).

NON-exhaustive sites that DON'T need touching (they have `_`/`if let`/`matches!`): `props_access.rs` `read_ts`/`with_ts_mut` (`_ => None`/`{}`), `repository/stream_assets.rs` asset-ref guard, and the per-kind `<Show when=…>`/`matches!`/`if let` reads inside `element_form.rs` (they only run for their own kind's draft). Test layers to add: core `validate_props` units (round-trip + tag + reject junk), a persistence `load_output_def` round-trip, a router create/patch (StreamElementDef serializes camelCase `{id, zOrder, props}`; the flattened variant fields are snake_case), and an editor E2E. E2E stacking check: element ↑/↓ "up" moves an element to a LOWER z_order (earlier in the def array = rendered BEHIND) — verified against the existing z-order test.

## Uploaded web fonts (#778) — table + FontStore + read-fonts + font-wait

Custom fonts are RUNTIME data served by Presenter (repo is PUBLIC — font bytes NEVER in git; only a tiny OFL fixture `tests/e2e/fixtures/fonts/Gruppo-Regular.ttf` + `OFL.txt`). Allowed font families = built-in `STREAM_FONT_FAMILIES` ∪ uploaded families.

- **`validate_props(props, extra_families: &[String])`** — core stays pure; the repository loads `distinct_font_families()` and passes it before every element create/update. `validate_text_style(style, extra_families)` unions the built-in whitelist with `extra_families`. Only prod callers: `repository/stream.rs` create/update. presenter-ui does NOT call `validate_props`.
- **Storage mirrors `stream_assets`:** `stream_fonts` table (one row per FACE = family+weight+italic; sha256 UNIQUE; NO sync cols, NO FK — referenced by family NAME), `repository/stream_fonts.rs` (sha dedup; guarded delete → 409 `ConflictDetail` ONLY when removing the LAST face of an IN-USE family — the scan is by family name GLOBALLY across all elements), `state/stream_fonts.rs` `FontStore` over `<stream-assets>/fonts/`. The sha/atomic-write primitives (`store_content_addressed`/`read_content`/`remove_content`/`sweep_tmp_dir` + `is_valid_sha256`/`sha256_hex`) are EXTRACTED as free fns in `state/stream_assets.rs` and REUSED by both `AssetStore` and `FontStore` — never copy them.
- **Magic-byte gate:** accept only raw ttf (`00 01 00 00` / `true`) + otf (`OTTO`); reject `.ttc` (`ttcf`), woff/woff2 (`wOFF`/`wOF2`) → 422 (a browser can't `@font-face` a TTC and read-fonts can't read woff2). `"fonts"` is a RESERVED slug (`/stream/fonts` + `/stream/fonts.css` shadow `/stream/{slug}`).
- **`read-fonts` (Google fontations) API** (presenter-server ONLY, never core/WASM — bloats the WASM bundle; replaced the unmaintained `ttf-parser` per RUSTSEC-2026-0192 in a later commit). `use read_fonts::{FontRef, TableProvider};` + `read_fonts::types::NameId`, `read_fonts::tables::os2::SelectionFlags`, `read_fonts::tables::head::MacStyle`. `FontRef::new(bytes) -> Result<_, ReadError>`; the `name`/`os2`/`head` tables come from the `TableProvider` trait (`font.name()`/`font.os2()`/`font.head()` each `-> Result<_, ReadError>`). Family = `NameId::TYPOGRAPHIC_FAMILY_NAME` (16) → fallback `NameId::FAMILY_NAME` (1): iterate `name.name_record()` (a `&[NameRecord]`), filter `record.name_id() == want` (`NameId` is `PartialEq`) and `record.platform_id()` ∈ {0 Unicode, 3 Windows}, decode via `record.string(name.string_data()) -> Result<NameString, _>` then `NameString`'s `Display` (`.to_string()`). Weight = `os2.us_weight_class() -> u16` (clamp 1..=1000, default 400 when no OS/2). Italic = `os2.fs_selection().contains(SelectionFlags::ITALIC)`, falling back to `head.mac_style().contains(MacStyle::ITALIC)` when the OS/2 table is absent. Default (`std`) feature enables the `NameString` string decoding.
- **`document.fonts` font-wait (no fallback flash):** `/stream/fonts.css` is injected at RUNTIME (`components/stream/fonts.rs::ensure_fonts_css_link`, NOT index.html) with `font-display: block` + `no-cache`+ETag; `spawn_font_gate` preloads faces via `document.fonts.load` and flips `data-fonts-ready` on the canvas (CSS hides `.stream-element--{countdown,lyrics,verse}` until then, bounded 2 s). Needs web-sys feature `FontFaceSet`; `Document::fonts() -> FontFaceSet` (NOT Result); `FontFaceSet::load(&str) -> js_sys::Promise` (NOT Result). The countdown div ALWAYS renders (with the font style) even when idle → E2E reads its computed `font-family` on the output page.
- **Editor picker:** `TextStyleForm` takes a `fonts: RwSignal<Vec<StreamFont>>` prop (threaded from `ctx.fonts` at all 6 call sites); options = built-ins ∪ uploaded (each styled in its own face); an uploaded family's weight control becomes a `<select>` of the uploaded weights (built-ins keep the numeric input). Upload/delete UI = `editor_fonts.rs` (`FontPanel`, multi-file, mounted in `pages/stream_editor.rs`).
- **Test trap (shared `cache=shared` in-memory DB + family-global delete scan):** every font upload in a router test dedups the SAME fixture to ONE process-wide row, and the guarded delete scans families GLOBALLY — so a test that DELETES a font must seed a UNIQUE family (via `repository().insert_or_get_stream_font` + `font_store().store`), never the shared fixture, or it races the in-use element another test leaves behind. (Same spirit as `stream_assets` tests using distinct image BYTES per test.) Also the #616 test-only-import class bit here: `state/stream_fonts.rs` names `Path` only in `#[cfg(test)]` code → `#[cfg(test)] use std::path::Path;`.
- **Import script `scripts/ops/import-resolume-fonts.sh <base-url> [--dry-run]`** (+ `read-font-metadata.py`, stdlib sfnt name-table reader): pulls `C:/Windows/Fonts` via tar-over-ssh (`sshpass -f ~/.secrets/resolume-ssh`). Windows gotchas: the Fonts SPECIAL folder rejects `dir *.ttf` wildcards ("path not found") → enumerate plainly + filter in bash; `dir /b` output is CRLF → `tr -d '\r'`; extracted fonts are read-only → `chmod -R u+w` before `rm`. Skips `.ttc`/`.fon` + Microsoft/stock (name-table "Microsoft" + a stock-filename list, filtered LOCALLY). Dry-run vs resolume.lan (2026-09-18): 674 ttf/otf → 417 upload, 172 Microsoft-name, 85 stock-file, 34 .ttc, 192 .fon (some MS "Nova/Pro"/CJK families whose name table omits "Microsoft" leak through — owner reviews before the real import).

## Lower thirds / nameplates ("menovky", #779) — a NEW pattern class: runtime "transient content" separate from the element

The lower third splits WHO is on air from HOW it looks, which is different from every other element (whose content comes from a live pipeline the OUTPUT binds to):

- **`StreamElementProps::LowerThird` is the LOOK only** (bar/accent colours, `bar_opacity`, `accent_width_pct`, two `TextStyle`s, `padding_pct`, `AnimationPreset` = `slide_left|slide_up|wipe`, `in_ms`/`out_ms` ≤ `STREAM_TRANSITION_MAX_MS`, `auto_hide_s` 0..=`STREAM_NAMEPLATE_AUTO_HIDE_MAX_S`=120). It carries NO text and NO `content_transition` (the enter/leave IS the transition). Validation is `validate_lower_third` (extracted so `validate_props` stays under the fn-length gate). Exhaustive sites all got an arm — grep `StreamElementProps::LowerThird` (core `kind_str`/`font_families`/`validate_props`; editor `props_access` `read_ts`/`with_ts_mut` (new `TsSlot::LowerThird{Primary,Secondary}`), `read_transition`/`with_transition_mut` + `read_opacity`/`with_opacity_mut` NO-op arms (it has `bar_opacity`, not the top-level `opacity`), `read_frame`/`with_frame_mut`, `default_element_props` before `_ => Verse`; `editor_panel` add-button + `kind_label_sk`; `element_form` `<Show>` + `LowerThirdFields`; output `scene_render::render_element`).
- **The plate LIST is a NEW table `stream_nameplates`** (person plates only; the SONG plate is VIRTUAL — resolved at show time from the camera-crew stage snapshot's `song_name`/`presentation_name` + `library_name`, empty song → typed `NameplateError::EmptySong` (409, a STATE-layer error, NOT a `RepositoryError` — router downcasts it). FK → `stream_outputs` ON DELETE CASCADE + index; NO sync cols. `Nameplate` DTO always `kind: Person` for a row; `sort_order` maps the `position` column. The list is NOT part of `config_revision` — its own `StreamNameplatesChanged` live event drives the editor/Companion refetch, so plate CRUD never forces an output-def refetch.
- **WHICH plate is on air is IN-MEMORY runtime state** (`NameplateManager`, `state/stream_nameplates.rs`) — NEVER persisted (a restart must not pop a plate back on air, same as scene activation). Show/hide broadcast `LiveEvent::StreamNameplate { output, active: Option<ActiveNameplate> }` (activation-class, applied inline, no `config_revision` bump). The event carries the RESOLVED texts so the output page needs no lookup.
- **Auto-hide runs SERVER-side** (so Companion feedback + the output both turn off): a tokio task sleeps the max `auto_hide_s` over the output's `lower_third` elements, guarded by the plate's monotonic `seq` — a newer show / explicit hide bumps the active seq so the older task's `seq` check mismatches and no-ops. `auto_hide_s == 0` = stay until hidden.
- **Output animation** (`components/stream/element_lower_third.rs`) mirrors the scene crossfade: seq-keyed `PlateLayer` list, `@starting-style` enter + `--leaving` exit + a `duration_ms` read reactively by seq (in_ms on create, out_ms on `mark_leaving`) + a JS removal buffer. Preset chosen by a `data-animation` attr → CSS (transform/opacity/clip-path only, no `@keyframes`). Bar transparency is an `rgba()` background (NOT the plate's `opacity`, which the fade animates). `ws/stream.rs` parses `stream_nameplate` into a `NameplateMsg` mirror (like `StreamStateMsg`); cold-load = `GET /stream/api/outputs/{slug}/nameplates/active`. The `?preview=1` page ALSO installs a `nameplate_preview` postMessage listener (the editor "Prehrať" — a fake plate, no broadcast; sibling of `draft_preview`).
- **API:** `GET|POST /stream/api/outputs/{slug}/nameplates`, `PATCH|DELETE /stream/api/nameplates/{id}`, `PUT /stream/api/outputs/{slug}/nameplates/order {ids}` (full set), `GET|PUT /stream/api/outputs/{slug}/nameplates/active` (`{source:"person",id}` | `{source:"song"}` | `{source:null}` = hide).
- **Companion** got its FIRST server-populated dropdown + presets: a `nameplates {output, plates:[{id,name,role}]}` OutgoingMessage (sent on connect + `StreamNameplatesChanged` + lag recovery) feeds the plugin's dropdown choices + DYNAMIC per-plate variable defs (`nameplate_<id>_name/role`) + `setPresetDefinitions` (one preset per plate + song; button text `$(presenter:nameplate_<id>_name)`; down = toggle; feedback `stream_nameplate_active` matched against `nameplate_active_id`). The plugin's `applyVariablesMessage` allowlist is DYNAMIC (`_variableIds()` = static ∪ per-plate) so the per-plate values are accepted. A show+song action REMAPS to the `stream_nameplate_song` wire command (`buildNameplateInvocation`). `nameplate_song_name/role` derive from the existing stage `song_name`/`band_name` variables (no new state).

## Timer overlay as a stream output — letter spacing + countdown box + timer_id honoured (#785)
