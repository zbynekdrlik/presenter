...
| `PRESENTER_STARTUP_MODE`      | `normal`                    | `normal` (default) boots every integration + background task. `validate` (#771) is the deploy "Validate database schema" probe mode: run migrations, open the DB, and serve `/healthz` on `PRESENTER_PORT`, but start NO integration — no NDI source restore, Companion websocket, OSC listener, AbleSet/Resolume pollers, song-sync loop, Android stage launcher, or background tickers — and `/healthz` returns the cold AI placeholder without probing the AI backend. Lets the probe run against a COPY of the live DB while the real service is running, without colliding on the fixed Companion port (18175) or duplicating the NDI pipeline. Case-insensitive; any unrecognised value → `normal` (with a WARN). On a migration/DB failure the binary prints a `SCHEMA-VALIDATION-FAILED:` marker so the deploy step distinguishes a broken migration from another startup failure. |
