...
RUN_AS_ORIGINAL STAGE_URL="${DEMO_ORIGIN}" AUDIT_LIBS="${AUDIT_LIBS:-NEW LEVEL}" AUDIT_MAX_PRESENTATIONS="${AUDIT_MAX_PRESENTATIONS:-0}" AUDIT_MAX_SLIDES="${AUDIT_MAX_SLIDES:-0}" STAGE_NEGATIVE="${STAGE_NEGATIVE:-1}" PLAYWRIGHT_WORKERS="${PLAYWRIGHT_WORKERS:-2}" npx playwright test --grep "$GREP_EXPR" --workers "${PLAYWRIGHT_WORKERS:-2}"
