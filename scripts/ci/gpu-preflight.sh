...
#      compute/graphics process AND tiny memory used (<=64 MiB) — wedged clocks.
#
...
#
# TESTABILITY: a real wedge can't be produced in CI. Set GPU_PREFLIGHT_FAKE=1 to
# enter mock mode (no live tool calls); the three facts are then injected via:
...
probe_nvenc() {
    ...
    rm -f "${HOME:-/root}/.cache/gstreamer-1.0/registry.x86_64.bin" 2>/dev/null || true
...
read_smi_utilmem() {
    ...
    nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits 2>/dev/null | head -1 || true
}
...
read_dmesg() {
    ...
    # dmesg may require sudo; tolerate failure (used only to enrich the message).
...
# Parse util / mem as integers (default 0 if unparseable).
UTIL="$(printf '%s' "$UTILMEM" | sed -n 's/^[[:space:]]*\([0-9]\+\).*/\1/p')"
MEM="$(printf '%s' "$UTILMEM" | sed -n 's/^[[:space:]]*[0-9]\+[[:space:]]*,[[:space:]]*\([0-9]\+\).*/\1/p')"
UTIL="${UTIL:-0}"
MEM="${MEM:-0}"
